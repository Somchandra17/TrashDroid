"""
Runtime hardening tests for PII initialization and logcat lifecycle cleanup.
"""
...
class TestBackgroundLogcatCollectorLifecycle(unittest.TestCase):
    @patch("utils.logcat_collector.subprocess.Popen")
    def test_stop_force_kills_stubborn_process_and_is_idempotent(self, popen_mock):
        proc = _StubbornProcess()
        popen_mock.return_value = proc

        with tempfile.TemporaryDirectory() as tmp:
            collector = BackgroundLogcatCollector("device", "com.example.app", Path(tmp))
            collector.start()
            time.sleep(0.05)
            collector.stop()
...
class TestForegroundLogcatMonitoringLifecycle(unittest.TestCase):
    @patch("phases.logcat.subprocess.Popen")
    @patch("phases.logcat.presidio_scan_text", return_value=[])
    @patch("phases.logcat.time.sleep", return_value=None)
    def test_phase_v_forces_teardown_even_when_readline_blocks(
        self, _sleep_mock, _scan_mock, popen_mock
    ):
        proc = _StubbornProcess()
        popen_mock.return_value = proc
        ...
        class DummyConfig:
            ...
            def log_command(self, *_args, **_kwargs):
                return None

            def add_finding(self, *_args, **_kwargs):
                return None

        with tempfile.TemporaryDirectory() as tmp:
            cfg = DummyConfig(Path(tmp))
            original_timeout = core_config.TIMING.logcat_auto_timeout
            core_config.TIMING.logcat_auto_timeout = 0
            try:
                run_logcat_monitoring(cfg, DummyADB())
            finally:
                core_config.TIMING.logcat_auto_timeout = original_timeout
...
class TestRuntimeCleanupManager(unittest.TestCase):
    ...
    def test_cleanup_is_one_shot_and_stops_collectors_once(self, _print_mock, reporter_cls):
        ...
        manager = RuntimeCleanupManager(screenshotter=screenshotter, config=cfg, device_info={"sdk": "34"})
        ...
        screenshotter.stop_scrcpy.assert_called_once()
